...
1. Run `dot skills updates --json` to see which imported skills have upstream changes.
...
5. Distinguish between two categories in the output:
   ...
   **Clean updates** (no `# local-edits:` in frontmatter):
   ...
   - For each accepted update, run `dot skills updates --update --skill <name> --no-commit` to apply it.
...
7. After applying any updates, run `dot stow` and `opencode debug skill` to verify.
