...
| `git-workflow` | Patterns for working with git branches, remotes, and diffs against the default branch | `branch-context` |
...
| Command | Description | Agent | Requires |
|---|---|---|---|
| `/check-skill-updates` | Check imported skills for upstream updates | default |  |
| `/cleanup-unnecessary-variables` | Inline and remove unnecessary variables from current git scope | refactorer | `branch-context` |
...
| `/git-workflow` | Read branch, diff, and PR context from BranchContextPlugin without extra git calls | ask | `branch-context` |
...
| `/refactor-current-work` | Refactor current branch work while preserving behaviour | refactorer | `branch-context` |
| `/remove-single-use-functions` | Inline and remove safe single-use functions from current git scope | refactorer | `branch-context` |
| `/reset-branch-reapply` | Reset branch to default and reapply current diff staged | build | `branch-context` |
| `/review-current-work` | Review current branch work with BranchContextPlugin context | reviewer | `branch-context` |
| `/types-enforce-ts` | Enforce TypeScript type safety in current git scope | refactorer | `branch-context` |
