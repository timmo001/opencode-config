...
| `git-commit` | Commit workflow using the dot git-commit gateway in the maintainer's concise one-line style. Use when creating a git commit, running /commit or /commit-push, staging and committing current changes, or pushing a just-made commit. Never run raw git commit; route through dot git-commit. |  | `writing-style` skill |
