...
function readOnlyByWorkspaceProbes(ruleset: readonly PermissionRule[]): boolean {
  return (
...
function allowsOrAsksTypicalWorkspaceEdits(ruleset: readonly PermissionRule[]): boolean {
...
export const ReadonlySubagentTaskGuard = (async ({ client, directory }) => {
  const query: DirectoryQuery | undefined = directory ? { directory } : undefined
